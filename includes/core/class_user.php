...
class User {
    ...
    public static $allow_search_types = [
        'phone',
        'first_name',
        'email',
    ];
    ...
    public static function users_list($d = []) {
        ...
        $search_type = isset($d['search_type']) && trim($d['search_type']) ? $d['search_type'] : '';
        ...
        if ($search) {
            $search_type = in_array($search_type, self::$allow_search_types) ? $search_type : current(self::$allow_search_types);
    ...
    public static function user_edit_update($d = []) {
        ...
        $email = isset($d['email']) && trim($d['email']) ? trim(mb_strtolower($d['email'])) : '';
        $plot_id = isset($d['plot_id']) && preg_match('/^\d+(, ?\d+)*$/', $d['plot_id']) ? trim($d['plot_id']) : '';
        
        $offset = isset($d['offset']) ? preg_replace('~\D+~', '', $d['offset']) : 0;

        if (mb_strlen($first_name) === 0 || mb_strlen($last_name) === 0 || mb_strlen($phone) === 0 || mb_strlen($email) === 0) {
            return ['html' => 'Не заполнены обязательные поля'];
        }
